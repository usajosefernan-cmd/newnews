...
export async function findEvidence(claimText, topicId, strategy = {}) {
  ...
  // Realizar las búsquedas web reales en DuckDuckGo en paralelo/secuencial
  const rawResults = [];
  for (const query of queries.slice(0, 2)) { // Limitar a las 2 queries principales para optimizar velocidad
    const res = await searchWebReal(query);
    rawResults.push(...res);
  }
  ...
  const prompt = `
Eres un Evaluador y Clasificador de Evidencias de Newnews España.
...
`;
  ...
  try {
    const evaluation = await callGemini(prompt, '06');
